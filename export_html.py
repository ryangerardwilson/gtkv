...
def _build_html(
    ...
    return (
        ...
        f"      .block-latex {{ font-size: {font.export_latex}; color: var(--latex-color); overflow-x: hidden; }}\n"
        "      .block-latex .katex-display { margin: 0.75em 0; }\n"
        "      .block-latex.latex-overflow { overflow-x: auto; -webkit-overflow-scrolling: touch; }\n"
        "      .theme-toggle {\n"
        "        position: fixed;\n"
        "        top: 16px;\n"
        "        right: 16px;\n"
        ...
        f"{map_items_light}"
        "      const fitLatexBlock = (el) => {\n"
        "        if (!el) return;\n"
        "        const display = el.querySelector('.katex-display');\n"
        "        if (!display) return;\n"
        "        const formula = display.querySelector('.katex') || display;\n"
        "        display.style.fontSize = '1em';\n"
        "        el.classList.remove('latex-overflow');\n"
        "        const availableWidth = el.clientWidth;\n"
        "        const formulaWidth = formula.scrollWidth;\n"
        "        if (!availableWidth || !formulaWidth) return;\n"
        "        let scale = availableWidth / formulaWidth;\n"
        "        if (scale >= 1) return;\n"
        "        scale = Math.max(0.6, scale);\n"
        "        display.style.fontSize = `${scale}em`;\n"
        "        if (formula.scrollWidth > availableWidth + 1) {\n"
        "          el.classList.add('latex-overflow');\n"
        "        }\n"
        "      };\n"
        "      const fitAllLatexBlocks = () => {\n"
        "        document.querySelectorAll('.block-latex').forEach((el) => fitLatexBlock(el));\n"
        "      };\n"
        "      let latexFitScheduled = false;\n"
        "      const scheduleLatexFit = () => {\n"
        "        if (latexFitScheduled) return;\n"
        "        latexFitScheduled = true;\n"
        "        requestAnimationFrame(() => {\n"
        "          latexFitScheduled = false;\n"
        "          fitAllLatexBlocks();\n"
        "        });\n"
        "      };\n"
        ...
        "      }\n"
        "      scheduleLatexFit();\n"
        "      window.addEventListener('resize', scheduleLatexFit);\n"
        "      window.addEventListener('orientationchange', scheduleLatexFit);\n"
        "      if (document.fonts && document.fonts.ready) {\n"
        "        document.fonts.ready.then(scheduleLatexFit).catch(() => {});\n"
        "      }\n"
        ...
        "        return !!target.closest('a, button, input, textarea, select, label, summary, [contenteditable=\"\"], [contenteditable=\"true\"], .theme-toggle, .nav-button, .index-panel, .site-modal, .doc-modal, .block-map, .block-three, .block-pyimage');\n"
        "      };\n"
        "      const maybeHandleDoubleTap = (event) => {\n"
        "        if (!isCoarsePointer) return;\n"
        "        if (!event.changedTouches || event.changedTouches.length !== 1) return;\n"
        "        if (docModal && docModal.classList.contains('open')) return;\n"
        "        const target = event.target;\n"
        "        if (isInteractiveTapTarget(target)) {\n"
        "          lastTapAt = 0;\n"
        "          return;\n"
        "        }\n"
        "        const touch = event.changedTouches[0];\n"
        "        const now = event.timeStamp || Date.now();\n"
        "        const quickEnough = now - lastTapAt <= DOUBLE_TAP_MS;\n"
        "        const closeEnough = Math.abs(touch.clientX - lastTapX) <= MAX_MOVE_PX && Math.abs(touch.clientY - lastTapY) <= MAX_MOVE_PX;\n"
        "        if (quickEnough && closeEnough) {\n"
        "          lastTapAt = 0;\n"
        "          event.preventDefault();\n"
        "          if (siteModal && siteModal.classList.contains('open')) {\n"
        "            closeSiteIndex();\n"
        "          }\n"
        "          openDocIndex();\n"
        "          return;\n"
        ...
        "      }\n"
        "      document.addEventListener('touchend', maybeHandleDoubleTap, { passive: false });\n"
        "      if (indexBackdrop) {\n"
        "        indexBackdrop.addEventListener('click', closeDocIndex);\n"
        "      }\n"
        "      if (indexBody) {\n"
        "        indexBody.addEventListener('click', (event) => {\n"
        "          const target = event.target;\n"
